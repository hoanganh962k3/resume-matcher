...
# By default Resume-Matcher now defaults to using the OpenAI provider so
# you do not need a local Ollama model server to run the project.
...
LLM_PROVIDER="providerstring"  # set to "openai" to use OpenAI
LLM_API_KEY="key"             # generic key (kept for compatibility)
OPENAI_API_KEY="key"          # preferred when using OpenAI provider
LLM_BASE_URL="url"
LL_MODEL="modelID"
