				...
				try {
					...
					results.push({
						...
						skillComparison: skillComp.map((s: any) => ({
							...
							match_status: s.resume_mentions >= s.job_mentions ? 'perfect' :
										  s.resume_mentions > 0 ? 'partial' : 'missing'
						})),
						data: data.data
					});
				} catch (err) {
		...
		return (
			<BackgroundContainer className="min-h-screen" innerClassName="overflow-auto">
				<div className="flex flex-col items-center justify-center h-full">
					...
					<p className="text-gray-300 text-lg">Analyzing {jobIds.length} job{jobIds.length > 1 ? 's' : ''}...</p>
				...
				<div className="flex items-center justify-between mb-8">
					<div>
						...
						<p className="text-gray-400">
							Comparing {comparisons.length} job{comparisons.length > 1 ? 's' : ''} against your resume
						</p>
					</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
					{comparisons.map((comparison) => (
						<JobComparisonCard
							...
							skillComparison={comparison.skillComparison}						onClick={() => handleViewJobDetails(comparison)}						/>
					))}
				</div>
					...
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
						...
						<div>
							...
							<div className="text-2xl font-bold text-blue-400">
								{Math.round((comparisons.reduce((sum, c) => sum + c.newScore, 0) / comparisons.length) * 100)}%
							</div>
						</div>
						<div>
