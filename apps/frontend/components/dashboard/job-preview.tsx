...
export default function JobPreview({ jobData }: JobPreviewProps) {
  ...
  const companyProfile =
    typeof jobData.company_profile === 'string'
      ? JSON.parse(jobData.company_profile)
      : jobData.company_profile;
  const location =
    typeof jobData.location === 'string' ? JSON.parse(jobData.location) : jobData.location;
  const keyResponsibilities =
    typeof jobData.key_responsibilities === 'string'
      ? JSON.parse(jobData.key_responsibilities)
      : jobData.key_responsibilities;
  const qualifications =
    typeof jobData.qualifications === 'string'
      ? JSON.parse(jobData.qualifications)
      : jobData.qualifications;
  const extractedKeywords =
    typeof jobData.extracted_keywords === 'string'
      ? JSON.parse(jobData.extracted_keywords)
      : jobData.extracted_keywords;

  const companyName =
    companyProfile?.companyName || companyProfile?.company_name || 'Company Name Not Available';
      ...
      <div className="space-y-6">
        ...
        {responsibilities && responsibilities.length > 0 && (
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-300 text-sm">
              {(showAllResponsibilities ? responsibilities : responsibilities.slice(0, 5)).map(
                ...
                <li>
                  ...
                  >
                    {showAllResponsibilities
                      ? 'Show less'
                      : `+${responsibilities.length - 5} more...`}
                  </button>
                </li>
              )}
            </ul>
        ...
        {quals && quals.length > 0 && (
          <div>
            ...
            <ul className="list-disc list-inside space-y-1 text-gray-300 text-sm">
              {(showAllQualifications ? quals : quals.slice(0, 5)).map(
        ...
        {keywords && keywords.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(showAllKeywords ? keywords : keywords.slice(0, 15)).map(
