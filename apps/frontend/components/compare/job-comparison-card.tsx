...
export default function JobComparisonCard({
  ...
  return (
    <div className="bg-gray-900/70 p-6 rounded-lg border border-gray-800/50 transition-all hover:border-gray-700">
      {/* Header */}
      <div className="mb-6">
        ...
        <div className="bg-gray-800/50 p-4 rounded-lg">
          ...
          {scoreImprovement > 0 && (
            <div className="flex items-center text-xs text-green-400 mt-1">
              <TrendingUp className="h-3 w-3 mr-1" />+{Math.round(scoreImprovement * 100)}%
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {matchedSkills.length > 10 && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                setShowAllMatched(!showAllMatched);
              }}
            ...
            {skillComparison.slice(0, 5).map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-xs">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-gray-500">Resume: {item.resume_mentions}</span>
                  <span className="text-gray-500">|</span>
                  <span className="text-gray-400">Job: {item.job_mentions}</span>
                  <span
                    className={`ml-2 px-1.5 py-0.5 rounded text-[10px] ${
                      item.match_status === 'perfect'
